...
def YOLO(input_layer, NUM_CLASS, model='yolov4', num_detection_layer=1):
    if num_detection_layer == 1:
        if model == 'yolov4':
            return YOLOv4_more_tiny(input_layer, NUM_CLASS)
        elif model == 'yolov3':
            assert "model not defined"
    elif num_detection_layer == 2:
        if model == 'yolov4':
            return YOLOv4_tiny(input_layer, NUM_CLASS)
        elif model == 'yolov3':
            return YOLOv3_tiny(input_layer, NUM_CLASS)
    elif num_detection_layer == 3:
        if model == 'yolov4':
...
def filter_boxes2(box_xywh, box_pred, box_classprob, score_threshold=0.4, input_shape = tf.constant([cfg.TRAIN.INPUT_SIZE,cfg.TRAIN.INPUT_SIZE])):
    scores_max = tf.math.reduce_max(box_pred, axis=-1)
