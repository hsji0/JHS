...
class Dataset(object):
    ...
    def mosaic_aug(self, image, bboxes, xrange = [0.3, 0.7] , yrange = [0.3, 0.7] ):
        ...
        # print("@@@ image shape:{}".format(image.shape))        # (640, 640, 3)
        # print("@@@ bboxes shape:{}".format(bboxes.shape))      # (8, 5)
        if len(image) < 4:
            # print("batch size < 4,skip mosaic")
            return image, bboxes


        # cv2.imshow("111resize",  cv2.resize(image[0], (neww, newh)))
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        # print("output_image shape:{}".format(output_image.shape))
        # print("neww newh shape:{} {}".format(neww, newh))
        # print("bboxes[0,:,0] {}".format(bboxes[0]))
        mosaic_bboxes = []
    ...
    def __next__(self):
        with tf.device("/cpu:0"):
            ...
            num = 0
            # num_mosaic = min(self.num_batchs // 4
            if self.batch_count < self.num_batchs:
                while num < self.batch_size:
                    ...
                    if cfg.YOLO.NUM_YOLOLAYERS == 3:
                        ...
                        image, bboxes = self.parse_annotation(annotation)
                        # print("@@@\n bboxes shape:{}".format(bboxes.shape))   # (BBOXES NUM, 5)
                        (
                            label_mbbox,
                            mbboxes,
                        ) = self.preprocess_true_boxes(bboxes, self.num_detection_layers)
    ...
    def parse_annotation(self, annotation):
        ...
        # print("line check:{}".format(line[:3]))
        image_path = line[0]
        ...
        if self.dataset_type == "converted_coco":
            ...
            # print("@@@ bboxes content check:{}".format(bboxes))
        if self.data_aug:
            ...
            print("@@@ image shape(crop):{}".format(image.shape))
            image, bboxes = self.random_translate(
                np.copy(image), np.copy(bboxes)
            )
            print("@@@ image shape(trans):{}".format(image.shape))
            image = self.random_color(
                np.copy(image)
            )
            print("@@@ image shape(color):{}".format(image.shape))
            if self.mosaic == True:
    ...
    def preprocess_true_boxes(self, bboxes, num_detection_layers):
        print("preprocess @!@: {}".format(np.array(bboxes).shape))
        label = [
            np.zeros(
                (
        ...
        print(np.array(bboxes_xywh).shape)
        ...
        for bbox in bboxes:
            ...
            print("bbox_xywh:{} shape:{}".format(bbox_xywh, bbox_xywh.shape))


            bbox_xywh_scaled = (
                1.0 * bbox_xywh[np.newaxis, :] / self.strides[:, np.newaxis]
            )
            print("bbox_xywh_scaled:{} shape:{}".format(bbox_xywh_scaled, np.array(bbox_xywh_scaled).shape))  # (1,4)
            ...
            for i in range(num_detection_layers):
                ...
                print("anchors xy",np.array(anchors_xywh[:, 0:2]).shape)
                anchors_xywh[:, 2:4] = self.anchors[i]
                print("bbox_xywh_scaled[i, 0:2]", np.array(bbox_xywh_scaled[i, 0:2]).shape)  # bbox_xywh_scaled[i, 0:2] (2,)
                print("anchors_wh", self.anchors[i])
